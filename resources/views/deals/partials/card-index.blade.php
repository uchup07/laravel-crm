@component('laravel-crm::components.card')
    ...
    @component('laravel-crm::components.card-header')
        ...
        @slot('actions')
            <div id="filter_date_range" data-kt-daterangepicker="true" data-kt-daterangepicker-opens="left" class="btn btn-sm btn-light d-flex align-items-center px-4">
                <!--begin::Display range-->
        ...
        <table class="table mb-0 card-table table-hover">
            ...
            @foreach($deals as $deal)
                <tr class="has-link @if($deal->closed_status == 'won') table-success @elseif($deal->closed_status == 'lost') table-danger @endif" data-url="{{ url(route('laravel-crm.deals.show',$deal)) }}">
                    <td>{{ $deal->created_at->toDayDateTimeString() }}</td>
                    <td>{{ $deal->title }}</td>
