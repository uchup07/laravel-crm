...
class DealPolicy
{
    ...
    {
        if ($this->isEnabled() && ($user->hasPermissionTo('view crm deals') && $deal->user_owner_id == auth()->user()->id) || (auth()->user()->hasRole(['Owner','Admin','Manager']))) {
            return true;
        }
    }
    ...
    {
        if ($this->isEnabled() && ($user->hasPermissionTo('edit crm deals') && $deal->user_owner_id == auth()->user()->id) || (auth()->user()->hasRole(['Owner','Admin']))) {
            return true;
        }
    }
    ...
    {
        if ($this->isEnabled() && ($user->hasPermissionTo('delete crm deals') && $deal->user_owner_id == auth()->user()->id) || (auth()->user()->hasRole(['Owner','Admin']))) {
            return true;
        }
    }
