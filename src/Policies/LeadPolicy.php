...
class LeadPolicy
{
    ...
    {
        if ($this->isEnabled() && ($user->hasPermissionTo('view crm leads') && $lead->user_owner_id == auth()->user()->id) || (auth()->user()->hasRole(['Owner','Admin','Manager']))) {
            return true;
        }
    }
    ...
    {
        if ($this->isEnabled() && ($user->hasPermissionTo('edit crm leads') && $lead->user_owner_id == auth()->user()->id) || (auth()->user()->hasRole(['Owner','Admin']))) {
            return true;
        }
    }
    ...
    {
        if ($this->isEnabled() && ($user->hasPermissionTo('delete crm leads') && $lead->user_owner_id == auth()->user()->id) || (auth()->user()->hasRole(['Owner','Admin']))) {
            return true;
        }
    }
